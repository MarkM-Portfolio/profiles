 ...
 * @author sberajaw
 */
public class MinByteLengthValidator implements Serializable 
{

	/**
	 * 
	 */
	private static final long serialVersionUID = 8531090860499820064L;
			...
			Field field, ActionMessages errors, HttpServletRequest request) throws Exception 
	{
		...
		int minByteLength = Integer.parseInt(field.getVarValue("minbytelength"));
		...
		if (inputValue != null) {
			...
			int minChars = (int) Math.ceil(minByteLength/avgBytesPerChar);

			Arg[] args = field.getArgs("minbytelength");
			for (int i = 0; i < args.length; i++) {
				Arg arg = args[i];
				if (arg.getKey().equalsIgnoreCase("(minbytelength)")) {
					arg.setKey(Integer.toString(minChars));
				}
			}
