...
function size() {
	...
	if(image.indexOf("max")>=0){
		...
		document.getElementById('imgsize').src = "[¿¡ímágés/réstóré.gíf~~~~~~~~~~es]";
		document.getElementById('imgsize').alt= "[¿¡Réstóré~~~~es]";
		document.getElementById('Contents').innerHTML="";
	}else{
		parent.document.getElementById('myFrameset').setAttribute('cols', '30%,*');
		document.getElementById('imgsize').src = "[¿¡ímágés/máxímízé.gíf~~~~~~~~~~es]";
		document.getElementById('imgsize').alt= "[¿¡Máxímízé~~~~es]";
		document.getElementById('Contents').innerHTML="[¿¡Cóñtéñts~~~~es]";
	}
}
...
function change_src(file, titleNew){
	...
	if(url>0){
		...
		parent.location = page +'[¿¡?dóc/~~es]' + file;
		
	}else{
		parent.location = current + "[¿¡?dóc/~~es]" + file;
	}
	var agt=navigator.userAgent.toLowerCase();
	if (agt.indexOf("safari") != -1){
		parent.mainpage.location = "[¿¡dóc/~es]" + file;
	}
}
