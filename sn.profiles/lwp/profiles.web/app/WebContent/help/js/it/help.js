...
function size() {
	...
	if(image.indexOf("max")>=0){
		...
		document.getElementById('imgsize').src = "[€ìmàgés/réstòré.gìf~~~~~~~~~~~it]";
		document.getElementById('imgsize').alt= "[€Réstòré~~~~~it]";
		document.getElementById('Contents').innerHTML="";
	}else{
		parent.document.getElementById('myFrameset').setAttribute('cols', '30%,*');
		document.getElementById('imgsize').src = "[€ìmàgés/màxìmìzé.gìf~~~~~~~~~~~it]";
		document.getElementById('imgsize').alt= "[€Màxìmìzé~~~~~it]";
		document.getElementById('Contents').innerHTML="[€Cònténts~~~~~it]";
	}
}
...
function change_src(file, titleNew){
	...
	if(url>0){
		...
		parent.location = page +'[€?dòc/~~~it]' + file;
		
	}else{
		parent.location = current + "[€?dòc/~~~it]" + file;
	}
	var agt=navigator.userAgent.toLowerCase();
	if (agt.indexOf("safari") != -1){
		parent.mainpage.location = "[€dòc/~~it]" + file;
	}
}
