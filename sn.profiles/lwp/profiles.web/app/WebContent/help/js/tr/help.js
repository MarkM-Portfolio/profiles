...
function size() {
	...
	if(image.indexOf("max")>=0){
		...
		document.getElementById('imgsize').src = "[İıimağeş/reştöre.ğif~~~~~~~~~~tr]";
		document.getElementById('imgsize').alt= "[İıReştöre~~~~tr]";
		document.getElementById('Contents').innerHTML="";
	}else{
		parent.document.getElementById('myFrameset').setAttribute('cols', '30%,*');
		document.getElementById('imgsize').src = "[İıimağeş/maksimize.ğif~~~~~~~~~tr]";
		document.getElementById('imgsize').alt= "[İıMaksimize~~~tr]";
		document.getElementById('Contents').innerHTML="[İıÇöntentş~~~~tr]";
	}
}
...
function change_src(file, titleNew){
	...
	if(url>0){
		...
		parent.location = page +'[İı?döç/~~tr]' + file;
		
	}else{
		parent.location = current + "[İı?döç/~~tr]" + file;
	}
	var agt=navigator.userAgent.toLowerCase();
	if (agt.indexOf("safari") != -1){
		parent.mainpage.location = "[İıdöç/~tr]" + file;
	}
}
