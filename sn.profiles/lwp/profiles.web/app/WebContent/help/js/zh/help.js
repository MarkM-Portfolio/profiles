...
function size() {
	...
	if(image.indexOf("max")>=0){
		...
		document.getElementById('imgsize').src = "[黒憒違images/restore.gif~~~~~~~~~zh]";
		document.getElementById('imgsize').alt= "[黒憒違Restore~~~zh]";
		document.getElementById('Contents').innerHTML="";
	}else{
		parent.document.getElementById('myFrameset').setAttribute('cols', '30%,*');
		document.getElementById('imgsize').src = "[黒憒違images/maximize.gif~~~~~~~~~zh]";
		document.getElementById('imgsize').alt= "[黒憒違Maximize~~~zh]";
		document.getElementById('Contents').innerHTML="[黒憒違Contents~~~zh]";
	}
}
...
function change_src(file, titleNew){
	...
	if(url>0){
		...
		parent.location = page +'[黒憒違?doc/~zh]' + file;
		
	}else{
		parent.location = current + "[黒憒違?doc/~zh]" + file;
	}
	var agt=navigator.userAgent.toLowerCase();
	if (agt.indexOf("safari") != -1){
		parent.mainpage.location = "[黒憒違doc/zh]" + file;
	}
}
