...
function size() {
	...
	if(image.indexOf("max")>=0){
		...
		document.getElementById('imgsize').src = "[בְתimages/restore.gif~~~~~~~~~iw]";
		document.getElementById('imgsize').alt= "[בְתRestore~~~iw]";
		document.getElementById('Contents').innerHTML="";
	}else{
		parent.document.getElementById('myFrameset').setAttribute('cols', '30%,*');
		document.getElementById('imgsize').src = "[בְתimages/maximize.gif~~~~~~~~~iw]";
		document.getElementById('imgsize').alt= "[בְתMaximize~~~iw]";
		document.getElementById('Contents').innerHTML="[בְתContents~~~iw]";
	}
}
...
function change_src(file, titleNew){
	...
	if(url>0){
		...
		parent.location = page +'[בְת?doc/~iw]' + file;
		
	}else{
		parent.location = current + "[בְת?doc/~iw]" + file;
	}
	var agt=navigator.userAgent.toLowerCase();
	if (agt.indexOf("safari") != -1){
		parent.mainpage.location = "[בְתdoc/~iw]" + file;
	}
}
