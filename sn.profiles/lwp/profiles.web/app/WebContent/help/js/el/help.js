...
function size() {
	...
	if(image.indexOf("max")>=0){
		...
		document.getElementById('imgsize').src = "[Άΐϊmάgέs/rέstόrέ.gϊf~~~~~~~~~~el]";
		document.getElementById('imgsize').alt= "[ΆΐRέstόrέ~~~~el]";
		document.getElementById('Contents').innerHTML="";
	}else{
		parent.document.getElementById('myFrameset').setAttribute('cols', '30%,*');
		document.getElementById('imgsize').src = "[Άΐϊmάgέs/mάχϊmϊzέ.gϊf~~~~~~~~~~el]";
		document.getElementById('imgsize').alt= "[ΆΐΜάχϊmϊzέ~~~~el]";
		document.getElementById('Contents').innerHTML="[ΆΐCόηtέηts~~~~el]";
	}
}
...
function change_src(file, titleNew){
	...
	if(url>0){
		...
		parent.location = page +'[Άΐ?dός/~~el]' + file;
		
	}else{
		parent.location = current + "[Άΐ?dός/~~el]" + file;
	}
	var agt=navigator.userAgent.toLowerCase();
	if (agt.indexOf("safari") != -1){
		parent.mainpage.location = "[Άΐdός/~el]" + file;
	}
}
