...
function size() {
	...
	if(image.indexOf("max")>=0){
		...
		document.getElementById('imgsize').src = "[Ůůímágěš/řěšťóřě.gíf~~~~~~~~~~cs]";
		document.getElementById('imgsize').alt= "[ŮůŘěšťóřě~~~~cs]";
		document.getElementById('Contents').innerHTML="";
	}else{
		parent.document.getElementById('myFrameset').setAttribute('cols', '30%,*');
		document.getElementById('imgsize').src = "[Ůůímágěš/máxímížě.gíf~~~~~~~~~~cs]";
		document.getElementById('imgsize').alt= "[ŮůMáxímížě~~~~cs]";
		document.getElementById('Contents').innerHTML="[ŮůČóňťěňťš~~~~cs]";
	}
}
...
function change_src(file, titleNew){
	...
	if(url>0){
		...
		parent.location = page +'[Ůů?ďóč/~~cs]' + file;
		
	}else{
		parent.location = current + "[Ůů?ďóč/~~cs]" + file;
	}
	var agt=navigator.userAgent.toLowerCase();
	if (agt.indexOf("safari") != -1){
		parent.mainpage.location = "[Ůůďóč/~cs]" + file;
	}
}
