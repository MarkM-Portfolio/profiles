...
function size() {
	...
	if(image.indexOf("max")>=0){
		...
		document.getElementById('imgsize').src = "[Лъфiмаgзs/язsтояз.gif~~~~~~~~~ru]";
		document.getElementById('imgsize').alt= "[ЛъфЯзsтояз~~~ru]";
		document.getElementById('Contents').innerHTML="";
	}else{
		parent.document.getElementById('myFrameset').setAttribute('cols', '30%,*');
		document.getElementById('imgsize').src = "[Лъфiмаgзs/мажiмizз.gif~~~~~~~~~ru]";
		document.getElementById('imgsize').alt= "[ЛъфМажiмizз~~~ru]";
		document.getElementById('Contents').innerHTML="[ЛъфСойтзйтs~~~ru]";
	}
}
...
function change_src(file, titleNew){
	...
	if(url>0){
		...
		parent.location = page +'[Лъф?dос/~ru]' + file;
		
	}else{
		parent.location = current + "[Лъф?dос/~ru]" + file;
	}
	var agt=navigator.userAgent.toLowerCase();
	if (agt.indexOf("safari") != -1){
		parent.mainpage.location = "[Лъфdос/ru]" + file;
	}
}
