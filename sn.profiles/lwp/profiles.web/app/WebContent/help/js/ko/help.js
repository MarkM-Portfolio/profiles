...
function size() {
	...
	if(image.indexOf("max")>=0){
		...
		document.getElementById('imgsize').src = "[한글images/restore.gif~~~~~~~~~~ko]";
		document.getElementById('imgsize').alt= "[한글Restore~~~~ko]";
		document.getElementById('Contents').innerHTML="";
	}else{
		parent.document.getElementById('myFrameset').setAttribute('cols', '30%,*');
		document.getElementById('imgsize').src = "[한글images/maximize.gif~~~~~~~~~~ko]";
		document.getElementById('imgsize').alt= "[한글Maximize~~~~ko]";
		document.getElementById('Contents').innerHTML="[한글Contents~~~~ko]";
	}
}
...
function change_src(file, titleNew){
	...
	if(url>0){
		...
		parent.location = page +'[한글?doc/~~ko]' + file;
		
	}else{
		parent.location = current + "[한글?doc/~~ko]" + file;
	}
	var agt=navigator.userAgent.toLowerCase();
	if (agt.indexOf("safari") != -1){
		parent.mainpage.location = "[한글doc/~ko]" + file;
	}
}
