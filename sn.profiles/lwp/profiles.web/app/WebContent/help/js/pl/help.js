...
function size() {
	...
	if(image.indexOf("max")>=0){
		...
		document.getElementById('imgsize').src = "[Żżimągęś/ręśtórę.gif~~~~~~~~~~pl]";
		document.getElementById('imgsize').alt= "[ŻżRęśtórę~~~~pl]";
		document.getElementById('Contents').innerHTML="";
	}else{
		parent.document.getElementById('myFrameset').setAttribute('cols', '30%,*');
		document.getElementById('imgsize').src = "[Żżimągęś/mąximiźę.gif~~~~~~~~~~pl]";
		document.getElementById('imgsize').alt= "[ŻżMąximiźę~~~~pl]";
		document.getElementById('Contents').innerHTML="[ŻżĆóńtęńtś~~~~pl]";
	}
}
...
function change_src(file, titleNew){
	...
	if(url>0){
		...
		parent.location = page +'[Żż?dóć/~~pl]' + file;
		
	}else{
		parent.location = current + "[Żż?dóć/~~pl]" + file;
	}
	var agt=navigator.userAgent.toLowerCase();
	if (agt.indexOf("safari") != -1){
		parent.mainpage.location = "[Żżdóć/~pl]" + file;
	}
}
