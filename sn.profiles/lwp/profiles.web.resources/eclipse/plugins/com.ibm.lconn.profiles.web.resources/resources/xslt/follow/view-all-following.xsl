...
<xsl:stylesheet version="1.0" 
  ...
  <xsl:variable name="newUI" select="$isNewUI = 1 or $isNewUI = 'true'" />
  
  <xsl:variable name="total-items-count" select="atom:feed/@snx:total-count" />
  ...
  <xsl:variable name="js-app-prefix" select="'lconn.profiles.Following'" />
  
  <xsl:param name="follow_noContacts" />
  <xsl:param name="follow_noFollowerContacts" />
  <xsl:param name="follow_fullPageTitle"/>
  <xsl:param name="label_inactive_user_msg"/>
  <xsl:param name="follow_title_ContactsFollowing" select="'Contacts Following {0}'"/>
  <xsl:param name="follow_title_ContactsFollowedBy" select="'Contacts Followed By {0}'"/>
  <xsl:param name="follow_title_Following" select="'{0} Following'"/>
         ...
         <xsl:when test="contains(atom:feed/@snx:ui-level,'second')">
          <div>
            <div id="followTabContent">
              <form onsubmit="return {$js-app-prefix}.removeContacts(this);">
  ...
  <xsl:param name="follow_sortByDisplayName"/>
  <xsl:param name="follow_sortByRecent"/>
  <xsl:param name="follow_telephone"/>
  <xsl:param name="follow_email"/>
  <xsl:param name="follow_altEmail"/>  
  <xsl:param name="follow_removedSelected"/>
  <xsl:param name="follow_removedSelectedcnx8"/>
  
  <xsl:param name="follow_tableheading_selection"/>
  <xsl:param name="follow_tableheading_photo"/>
  <xsl:param name="follow_tableheading_employeeinfo"/>  
  <xsl:param name="follow_tableheading_contactinfo"/>  
    
  <xsl:template name="third-level-content">
    <div id="followsThirdLevel" class="lotusChunk" data-total-count="{$total-items-count}">
      <xsl:choose>
        <xsl:when test="$total-items-count > 0">
          <xsl:choose>
            <xsl:when test="$newUI">
              <div id="pageItemsMainRegion" aria-label="" role="region">
                <table id="follow_mainContentTable" class="lotusTable" border="0" cellspacing="0" width="100%">
                  <tbody id="followingcnx8">			
                    <xsl:for-each select="atom:feed/atom:entry">
                      ...
                      <td>
                        ...
                        <div>                        
                        <div><xsl:value-of select="@snx:tel" /></div>
                          <xsl:if test="@snx:email != ''">
                          <div><strong><a href="mailto:{@snx:email}"><xsl:value-of select="@snx:email" /></a></strong></div>
                          </xsl:if>
                          <xsl:if test="@snx:groupware-email != ''">
                          <div><xsl:value-of select="$follow_altEmail"/>&#160;<strong><a href="mailto:{@snx:groupware-mail}"><xsl:value-of select="@snx:groupware-mail" /></a></strong></div>
                          </xsl:if>
                        </div>
                        <xsl:if test="$isCurrentUser">
                        <xsl:if test="$action = 'in'">
                          <div id="follow_removeContactsDiv" class="lotusChunk lotusBtnContainer">
                            <a href="javascript:void(0);" role="button" id="{@snx:key}" value="select_contact_{$pos}" aria-label="{atom:title/text()}"
                              onclick="{$js-app-prefix}.removeContact(this);">
                              <xsl:value-of select="$follow_removedSelectedcnx8"/>
                            </a>
                          </div>
                        </xsl:if>
                        </xsl:if>
              ...
              <xsl:call-template name="table-paging-cnx8ui"/>
            </xsl:when>
            <xsl:otherwise>
              <xsl:if test="$isCurrentUser">
              <xsl:if test="$action = 'in'">
              <div id="follow_removeContactsDiv" class="lotusChunk lotusBtnContainer">
                ...
                <a href="javascript:void(0);" role="button"
                  onclick="{$js-app-prefix}.removeContacts(this);">
                  <xsl:value-of select="$follow_removedSelected"/>
                </a>
                </span>
                <br/><br/>
              </div>
                ...
                <table id="follow_mainContentTable" class="lotusTable" border="0" cellspacing="0" width="100%">
                  <tbody>
                    <tr class="lotusHidden"><!-- a11y -->
                      <th><xsl:value-of select="$follow_tableheading_selection"/></th>
                      <th><xsl:value-of select="$follow_tableheading_photo"/></th>
                      <th><xsl:value-of select="$follow_tableheading_employeeinfo"/></th>
                      <th><xsl:value-of select="$follow_tableheading_contactinfo"/></th>
                    </tr>				
                  <xsl:for-each select="atom:feed/atom:entry">
                    <xsl:variable name="pos" select="position()" />
                    <tr>  
                    ...
                    <xsl:if test="$isCurrentUser">
                    <xsl:if test="$action = 'in'">
                      <td class="lotusFirstCell" style="width:25px">
                      <input type="checkbox" value="{@snx:key}"  id="select_contact_{$pos}" aria-label="{atom:title/text()}"/>
                      </td>
                    </xsl:if>
                    </xsl:if>
                    <td style="width:70px">
                    ...
                    <td class="lotusLastCell">
                      <div>                        
                      <div><xsl:value-of select="$follow_telephone"/>&#160;<xsl:value-of select="@snx:tel" /></div>
                      <xsl:if test="$showEmail = 'true' ">
                        <xsl:if test="@snx:email != ''">
                        <div><xsl:value-of select="$follow_email"/>&#160;<strong><a href="mailto:{@snx:email}"><xsl:value-of select="@snx:email" /></a></strong></div>
                        </xsl:if>
                        <xsl:if test="@snx:groupware-email != ''">
                        <div><xsl:value-of select="$follow_altEmail"/>&#160;<strong><a href="mailto:{@snx:groupware-mail}"><xsl:value-of select="@snx:groupware-mail" /></a></strong></div>
                        </xsl:if>
          ...
          <xsl:otherwise>
			<xsl:choose>
				<xsl:when test="$action = 'in'">
					<xsl:value-of select="$follow_noContacts" />
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="$follow_noFollowerContacts" />
				</xsl:otherwise>
			</xsl:choose>
  ...
  <!-- COPIED CODE FROM ../table-paging.xsl. TABLE-PAGING.XSL IS THE ORIGINAL FILE -->
	...
	<xsl:param name="tablePagingResultsPerPage" select="'Results per page'"/>
	...
	<xsl:param name="tablePagingFirstAlt" select="'Go to the first page of items'"/>
	<xsl:param name="tablePagingLastAlt" select="'Go to the last page of items '"/>
	...
	<xsl:template name="items-per-page-link-cnx8ui">
		<xsl:param name="count">5</xsl:param>
		<xsl:param name="class"></xsl:param>

		<div class="cnx8-ui-items-per-page-dropdown-item {$class}">
			<xsl:choose>
				<xsl:when test="$items-per-page = $count">
	...
	<xsl:template name="table-paging-cnx8ui">
		<!-- Bottom paging -->
  		<hr style="margin-bottom: 25px;margin-right: 10px;"/>
		<div class="lotusPaging" role="region" aria-label="{$tableBottomPagingLabel} {$paging-info-alt}">
			<xsl:call-template name="output-hidden-fields"/>
			
			<ul class="lotusLeft lotusInlinelist cnx8-ui-pull-to-center" role="list">
				<xsl:choose>         
					<xsl:when test="$current-page = 0">
						<li class="lotusFirst" role="listitem" aria-disabled="true"><div class="cnx8-ui-pagination cnx8-ui-pagination-first"></div></li>
					</xsl:when>             
					<xsl:otherwise>
					<li class="lotusFirst" role="listitem">
						<a href="javascript:void(0);" onclick="{$js-app-prefix}.pageTo(this, 0)" title="{$tablePagingFirstAlt}" aria-label="{$tablePagingFirstAlt}">
				...
				<xsl:choose>         
					<xsl:when test="$current-page = 0">
						<li class="" role="listitem" aria-disabled="true"><div class="cnx8-ui-pagination cnx8-ui-pagination-prev"></div></li>
					</xsl:when>             
					<xsl:otherwise>
				...
				<xsl:call-template name="display-pages-cnx8ui"/>

				<xsl:choose>         
					<xsl:when test="$current-page = $total-pages">
						<li role="listitem" aria-disabled="true"><div class="cnx8-ui-pagination cnx8-ui-pagination-next"></div></li>
					</xsl:when>             
					<xsl:otherwise>
						<li role="listitem">
							<a href="javascript:void(0);" onclick="{$js-app-prefix}.pageTo(this,{$current-page + 1})" title="{$tablePagingNextAlt}" aria-label="{$tablePagingNextAlt}">
								<div class="cnx8-ui-pagination cnx8-ui-pagination-next"></div>
							</a>
						</li>
					</xsl:otherwise>
				</xsl:choose>
				...
				<xsl:choose>         
					<xsl:when test="$current-page = $total-pages">
						<li role="listitem" aria-disabled="true"><div class="cnx8-ui-pagination cnx8-ui-pagination-last"></div></li>
					</xsl:when>             
					<xsl:otherwise>
						<li role="listitem">
							<a href="javascript:void(0);" onclick="{$js-app-prefix}.pageTo(this,{$total-pages})" title="{$tablePagingLastAlt}" aria-label="{$tablePagingLastAlt}">
								<div class="cnx8-ui-pagination cnx8-ui-pagination-last"></div>
							</a>
						</li>
					</xsl:otherwise>
				</xsl:choose>
			...
			<div class="cnx8-ui-pull-to-center" style="float:right">
				<span aria-hidden="true"><xsl:value-of select="$tablePagingResultsPerPage"/></span>
				
				<span id="selectedResultsPerPage"><xsl:value-of select="$items-per-page"/></span>

				<div class="cnx8-ui-items-per-page-dropdown-main">
					<ul class="cnx8-ui-items-per-page-dropdown-outer">
						<li><a href="javascript:void(0);"><div id="dropdown-selection"></div></a>
							<div class="cnx8-ui-items-per-page-dropdown-inner">	
								<xsl:call-template name="items-per-page-link-cnx8ui">
									<xsl:with-param name="count">5</xsl:with-param>
									<xsl:with-param name="class">lotusFirst</xsl:with-param>
								</xsl:call-template>
	...
	<xsl:template name="display-pages-cnx8ui">
