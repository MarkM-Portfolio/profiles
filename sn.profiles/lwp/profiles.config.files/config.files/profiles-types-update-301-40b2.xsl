...
<xsl:stylesheet version="1.0"
	...
	<!-- TODO handle the use case for codes in profileLayout -->
	...
	<xsl:template name="migrateLayout">
		...
		<xsl:for-each select="$jobInformation/*">
			<xsl:call-template name="migrateLayoutAttribute">
				...
				<xsl:with-param name="duplicates" select="$apiProfileTypes[@profileType=$layoutProfileType/@profileType]/tns:editableApiAttribute/. | $apiProfileTypes[@profileType=$layoutProfileType/@profileType]/tns:editableApiExtensionAttribute/@extensionIdRef"/>
			</xsl:call-template>			
		</xsl:for-each>	
		<xsl:for-each select="$contactInformation/*">
			<xsl:call-template name="migrateLayoutAttribute">
				<xsl:with-param name="attribute" select="."/>
				<xsl:with-param name="duplicates" select="$apiProfileTypes[@profileType=$layoutProfileType/@profileType]/tns:editableApiAttribute/. | $apiProfileTypes[@profileType=$layoutProfileType/@profileType]/tns:editableApiExtensionAttribute/@extensionIdRef | $jobInformation/tns:attribute/. | $jobInformation/tns:editableAttribute/. | $jobInformation/tns:extensionAttribute/@extensionIdRef"/>
			</xsl:call-template>			
		</xsl:for-each>		
		<xsl:for-each select="$associatedInformation/*">
			<xsl:call-template name="migrateLayoutAttribute">
				<xsl:with-param name="attribute" select="."/>
				<xsl:with-param name="duplicates" select="$apiProfileTypes[@profileType=$layoutProfileType/@profileType]/tns:editableApiAttribute/. | $apiProfileTypes[@profileType=$layoutProfileType/@profileType]/tns:editableApiExtensionAttribute/@extensionIdRef | $jobInformation/tns:attribute/. | $jobInformation/tns:editableAttribute/. | $jobInformation/tns:extensionAttribute/@extensionIdRef | $contactInformation/tns:attribute/. | $contactInformation/tns:editableAttribute/. | $contactInformation/tns:extensionAttribute/@extensionIdRef"/>
			</xsl:call-template>			
		</xsl:for-each>	
	</xsl:template>
	...
	<xsl:template name="migrateLayoutAttribute">
		...
		<xsl:variable name="isReadwrite" select="((local-name() = 'editableAttribute') or ($isExtensionAttribute and @editable='true'))"/>
		...
		<xsl:variable name="isDuplicate" select="boolean($duplicates[.=$ref])"/>
		<xsl:variable name="updatability">
			<xsl:choose>
				<xsl:when test="$isReadwrite">
					<xsl:text>readwrite</xsl:text>
