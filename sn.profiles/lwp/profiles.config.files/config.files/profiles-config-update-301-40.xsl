...
*****************************************************************
        ...
        <config xmlns="http://www.ibm.com/profiles-config" xmlns:tns="http://www.ibm.com/profiles-config"
            ...
            <dataAccess>
                <xsl:comment> indicates if organizational data is available (i.e. managerUid) </xsl:comment>            
            	<xsl:copy-of select="tns:dataAccess/tns:organizationalStructureEnabled"/>
                <xsl:comment> indicates which fields need to be resolved before being displayed </xsl:comment>
                <xsl:comment> WARNING: in most deployments the resolvedCodes section should not be modified </xsl:comment>            	
