...
REM set default code ahead of time in case called program doesn't set anything
echo 1 >".\_tdi.rc"

CALL "%TDIPATH%\ibmdisrv" -s . -c profiles_tdi.xml -r mark_managers
FOR /F "usebackq" %%i in (".\_tdi.rc") DO SET RC=%%i
IF NOT "%RC%" == "0" GOTO FAILPOP
GOTO FINISH

:FAILPOP
ECHO "Mark managers failed"
GOTO FINISH
