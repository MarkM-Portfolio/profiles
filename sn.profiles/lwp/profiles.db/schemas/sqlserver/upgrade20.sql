...
UPDATE EMPINST.GIVEN_NAME 
	SET PROF_KEY=
		...
		WHERE PROF_UID=GIVEN_NAME.PROF_KEY);
GO
...
CREATE INDEX GIVEN_NAME_IDX2 ON EMPINST.GIVEN_NAME 
		(PROF_GIVENNAME ASC, PROF_KEY ASC);
GO

CREATE INDEX GIVEN_NAME_WIZ1 ON EMPINST.GIVEN_NAME 
		(PROF_KEY DESC,
		 PROF_GIVENNAME DESC);
GO
...
UPDATE EMPINST.SURNAME 
	SET PROF_KEY=
		(SELECT PROF_KEY 
		FROM EMPINST.EMPLOYEE E 
		WHERE PROF_UID=SURNAME.PROF_KEY);
GO
...
CREATE INDEX SURNAME_UID_IDX ON EMPINST.SURNAME 
		(PROF_SURNAME ASC, PROF_KEY ASC) ;
GO

CREATE INDEX SURNAME_WIZ1 ON EMPINST.SURNAME 
		(PROF_KEY DESC,
		 PROF_SURNAME DESC);
GO
...
UPDATE EMPINST.PROFILE_EXTENSIONS 
	SET PROF_KEY=
		(SELECT PROF_KEY 
		FROM EMPINST.EMPLOYEE E 
		WHERE PROF_UID=PROFILE_EXTENSIONS.PROF_KEY);
GO
...
UPDATE EMPINST.PROFILE_EXT_DRAFT 
	SET PROF_KEY=
		(SELECT PROF_KEY 
		FROM EMPINST.EMPLOYEE E 
		WHERE PROF_UID=PROFILE_EXT_DRAFT.PROF_KEY);
GO
...
UPDATE EMPINST.PEOPLE_TAG 
	SET PROF_TARGET_KEY=
		(SELECT PROF_KEY 
		FROM EMPINST.EMPLOYEE E 
		WHERE PROF_UID=PEOPLE_TAG.PROF_TARGET_KEY);
GO
...
UPDATE EMPINST.PHOTO
	SET PROF_KEY=
		(SELECT PROF_KEY 
		FROM EMPINST.EMPLOYEE E 
		WHERE PROF_UID=PHOTO.PROF_KEY);
GO
...
exec sp_rename 'EMPINST.PRONUNCIATION.PRONOUNCE_PK', 'PRONOUNCE_PK_T'
exec sp_rename 'EMPINST.T_PRONOUNCE_INSRT', 'T_PRONOUNCE_T'
GO

--- step 2: rename old table to temp

exec sp_rename 'EMPINST.PRONUNCIATION', 'PRONUNCIATION_T'
GO
...
UPDATE EMPINST.PRONUNCIATION
	SET PROF_KEY=
		(SELECT PROF_KEY 
		FROM EMPINST.EMPLOYEE E 
		WHERE PROF_UID=PRONUNCIATION.PROF_KEY);
GO

--- step 4b: add constraints on new table

CREATE TRIGGER EMPINST.T_PRONOUNCE_INSRT
ON EMPINST.PRONUNCIATION
FOR UPDATE 
AS
    IF @@ROWCOUNT = 0
       BEGIN
    ...
    ELSE IF @@ROWCOUNT = 1
        ...
        DECLARE @n_id NVARCHAR(36)
        SELECT @n_id = PROF_KEY FROM inserted
        UPDATE EMPINST.PRONUNCIATION SET PROF_UPDATED = CURRENT_TIMESTAMP WHERE PROF_KEY = @n_id
        END
    ELSE
        BEGIN
        SET NOCOUNT ON;
        UPDATE EMPINST.PRONUNCIATION SET PROF_UPDATED = CURRENT_TIMESTAMP WHERE PROF_KEY IN (SELECT distinct PROF_KEY FROM inserted)
        END
GO
...
CREATE UNIQUE INDEX PRONOUNCE_PK ON EMPINST.PRONUNCIATION
		(PROF_KEY ASC) ;
GO
...
exec sp_rename 'EMPINST.CHG_EMP_DRAFT', 'CHG_EMP_DRAFT_T'
GO
...
SET IDENTITY_INSERT EMPINST.CHG_EMP_DRAFT ON

INSERT INTO EMPINST.CHG_EMP_DRAFT (
	IBMSNAP_COMMITSEQ,
	IBMSNAP_INTENTSEQ,
	IBMSNAP_LOGMARKER,
	IBMSNAP_OPERATION,
...
SET IDENTITY_INSERT EMPINST.CHG_EMP_DRAFT OFF
GO

UPDATE EMPINST.CHG_EMP_DRAFT
	SET PROF_KEY=
		(SELECT PROF_KEY 
		FROM EMPINST.EMPLOYEE E 
		WHERE PROF_UID=CHG_EMP_DRAFT.PROF_KEY);
GO
--- step 4b: add constraints on new table
...
CREATE SCHEMA SNCORE AUTHORIZATION dbo;
GO
...
CREATE TABLE EMPINST.PROF_STRUCT_TAG  (
	PROF_SOURCE_KEY 	NVARCHAR(36) NOT NULL , 
	PROF_TARGET_KEY 	NVARCHAR(36) NOT NULL,
	PROF_NODE_ID 		NUMERIC(19,0) NOT NULL
 ) ;
GO
...
CREATE TABLE EMPINST.PROF_CONNECTIONS (
		PROF_CONNECTION_ID NVARCHAR(36) NOT NULL PRIMARY KEY,
		PROF_SOURCE_KEY NVARCHAR(36) NOT NULL,
		PROF_TARGET_KEY NVARCHAR(36) NOT NULL,
		...
		PROF_VISIBILITY NUMERIC(19,0) NOT NULL,
		PROF_TYPE NUMERIC(19,0) NOT NULL,
		PROF_VALUE NVARCHAR(36) NOT NULL,
...
-- GRANT TO PROFUSER
--------------------------------------
GRANT DELETE,INSERT,SELECT,UPDATE ON EMPINST.PROF_STRUCT_TAG TO PROFUSER
GO
...
GO 
GRANT DELETE,INSERT,SELECT,UPDATE,CONTROL ON SNCORE.STRUCT_TAG TO PROFUSER
GO
