 ...
 * 
 */
public class AbstractConnectionCloudTest extends AbstractCloudTest {

	public void getAllConnectionsForMain() throws Exception {
		getAllConnections(orgAUserATransport, ColleagueConnection.STATUS_ALL, true);
	}

	public void getAllConnectionsForOther() throws Exception {
		getAllConnections(orgAUserBTransport, ColleagueConnection.STATUS_ALL, true);
	}
	...
	public static void deleteColleagueConnection(Transport t, Transport u, Collection<ColleagueConnection.STATUS> statuses, boolean verbose)
		...
		Service service = t.doAtomGet(Service.class, urlBuilder.getProfilesServiceDocument(), NO_HEADERS, null, null,
				false);
		...
		service = u.doAtomGet(Service.class, urlBuilder.getProfilesServiceDocument(), NO_HEADERS, null, null,
				false);
		...
		for (ColleagueConnection.STATUS s : statuses) {
			...
			for (ColleagueConnection e : colleagueFeedT.getEntries()) {
				if (userIdOther.equals(e.getSource().getUserId()) || userIdOther.equals(e.getTarget().getUserId())) {
					t.doAtomDelete(e.getEditLink(), AbstractCloudTest.NO_HEADERS, HTTPResponseValidator.OK);
				}
			}

			ColleagueFeed colleagueFeedU = new ColleagueFeed(u.doAtomGet(Feed.class,
					URLBuilder.addQueryParameter(colleagueLinkU, "status", s.name(), false), HTTPResponseValidator.OK, verbose)).validate();
			...
			for (ColleagueConnection e : colleagueFeedU.getEntries()) {
				if (userIdMain.equals(e.getSource().getUserId()) || userIdMain.equals(e.getTarget().getUserId()))
					u.doAtomDelete(e.getEditLink(), AbstractCloudTest.NO_HEADERS, HTTPResponseValidator.OK);
			}
		}
	}
	...
	public static ColleagueConnection getColleagueConnection(Transport source, Transport target,
		...
		ProfileService profilesServiceSource = ProfileService.parseFrom(source.doAtomGet(Service.class,
				AbstractCloudTest.urlBuilder.getProfilesServiceDocument(), HTTPResponseValidator.OK, false));
	...
	public static ColleagueConnection createColleagueConnection(Transport inviter, Transport invitee, String messageInvite,
		...
		return createColleagueConnection(null, null, inviter, invitee, messageInvite, messageAccept, HTTPResponseValidator.CREATED_NO_HEADER,
	...
	public static ColleagueConnection createColleagueConnection(Transport inviterImpersonator, Transport inviteeImpersonator,
		...
		if (verbose) prettyPrint(response);
		...
		if (!validatorCreate.isErrorExpected()) {
			...
			if (doComplete) {
				...
				// there is no respone body on this call
				...
				retVal = new ColleagueConnection(invitee.doAtomGet(Entry.class, retVal.getEditLink(), HTTPResponseValidator.OK, verbose))
						.validate();
			}
		}
		return retVal;
