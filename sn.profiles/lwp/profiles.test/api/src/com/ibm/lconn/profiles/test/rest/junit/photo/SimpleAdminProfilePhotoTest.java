...
public class SimpleAdminProfilePhotoTest extends BaseProfilePhotoTest {
...
//	}
	...
	public void testUpdateProfilePhoto() throws Exception
	{
		...
		int index = 0;
		ProfileEntry profileEntry = profileEntries.get(index);
		...
		verifyPhotoGet(adminTransport, profileEntry, URLBuilder.Query.IMAGE, expectedHTTPResponse);

		// verify that GET with URL param mcode= is NOT blocked
		 expectedHTTPResponse = HTTPResponseValidator.OK;
		 verifyPhotoGet(adminTransport, profileEntry, URLBuilder.Query.MCODE, expectedHTTPResponse);

		// verify that GET with URL param userid= is NOT blocked
		expectedHTTPResponse = HTTPResponseValidator.OK;
		verifyPhotoGet(adminTransport, profileEntry, URLBuilder.Query.USER_ID, expectedHTTPResponse);
		...
		validateImageDimensions(URLBuilder.updateLastMod(imageURL), otherTransport,
		...
		// as Admin update the Admin's photo
		expectedHTTPResponse = HTTPResponseValidator.OK;
		ProfileService adminProfilesService = ProfileService.parseFrom(adminTransport.doAtomGet(Service.class,
				urlBuilder.getProfilesServiceDocument(), NO_HEADERS, expectedHTTPResponse));
		Feed adminRawFeed = adminTransport.doAtomGet(Feed.class, adminProfilesService.getProfileFeedUrl(), NO_HEADERS, HTTPResponseValidator.OK);
		...
		adminPhoto = readPhoto("Easter_Island.jpg", 165, 165, false); // image is bigger than 165 X 165
		adminPhoto = readPhoto("bird165.jpg", 165, 165);
		
		ProfileEntry adminProfileEntryMcode = getAdminProfile(adminEmail);
		expectedHTTPResponse = HTTPResponseValidator.OK;
		verifyPhotoPut(adminTransport, adminProfileEntryMcode, URLBuilder.Query.MCODE, adminPhoto, expectedHTTPResponse);
	...
	private String checkPhotoUploadAndUpdate(String asUser, Transport transport, ProfileEntry profileEntry, String[] photos) throws Exception
	{
		HTTPResponseValidator expectedHTTPResponse = HTTPResponseValidator.OK;
		...
		validateImageDimensions(URLBuilder.updateLastMod(imageURL), transport,
				PHOTO_DEFAULT_HEIGHT, PHOTO_DEFAULT_WIDTH);
	...
	{
		InputStream photo = getResourceAsStream(SimpleAdminProfilePhotoTest.class, fileName);
		if (checkDimensions) {
			validateImageDimensions(photo, height, width);
			photo = getResourceAsStream(SimpleAdminProfilePhotoTest.class, fileName);
		}
		return photo;
	}
