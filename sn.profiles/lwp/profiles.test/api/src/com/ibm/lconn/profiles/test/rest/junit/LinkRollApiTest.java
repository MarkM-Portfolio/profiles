...
public class LinkRollApiTest extends AbstractTest {
	...
	public void testProfileLinks() throws Exception {
		...
		mainTransport.doAtomDelete(profileLinkHref, NO_HEADERS, HTTPResponseValidator.OK);
		
		// fetch the xml from server (it could return with a 204 meaning there is no content)
		Element linkRollXML = mainTransport.doAtomGet(Element.class, profileLinkHref, NO_HEADERS, ApiConstants.LinkRollConstants.MEDIA_TYPE, HTTPResponseValidator.NO_CONTENT, false);
		...
		mainTransport.doAtomPut(null, profileLinkHref, linkRoll.toElement(), ApiConstants.LinkRollConstants.MEDIA_TYPE, NO_HEADERS, HTTPResponseValidator.NO_CONTENT);

		profileLinkHref = profileLinkHref + "&foo=true";
		LinkRoll result = new LinkRoll(mainTransport.doAtomGet(Element.class, profileLinkHref, NO_HEADERS, ApiConstants.LinkRollConstants.MEDIA_TYPE, HTTPResponseValidator.OK, false));
