...
package com.ibm.lconn.profiles.test.rest.junit.pronunciation;
...
public class ProfilePronunciationTest extends AbstractTest {
	...
	public void testCRUD() throws Exception {
		...
		try {
			...
			int status = mainTransport.doStatusGet(url, NO_HEADERS);
			if (204 == status) {
				...
				mainTransport.doAtomGet(null, URLBuilder.updateLastMod(url), NO_HEADERS, HTTPResponseValidator.NO_CONTENT);
			}
			else {
			...
			audioFile = this.getClass().getResourceAsStream("7A7276897.wav");
			this.mainTransport.doAtomPut(null, URLBuilder.updateLastMod(url), audioFile, "image/jpeg", NO_HEADERS,
					HTTPResponseValidator.BAD_REQUEST);

			// submit with acceptable contentType this time
			audioFile = this.getClass().getResourceAsStream("7A7276897.wav");
			this.mainTransport.doAtomPut(null, URLBuilder.updateLastMod(url), audioFile, "audio/wav", NO_HEADERS,
					HTTPResponseValidator.OK);

			// verify the pronunciation exists
			byte[] afterBytes = mainTransport.doBytesGet(URLBuilder.updateLastMod(url), NO_HEADERS, HTTPResponseValidator.OK);
